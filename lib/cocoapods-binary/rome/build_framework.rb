...
def build_for_iosish_platform(sandbox, 
  ...
  xcodebuild(sandbox, target_label, CONFIGURATION_DEVICE, deployment_target, other_options)
  xcodebuild(sandbox, target_label, CONFIGURATION_SIMULATOR, deployment_target, other_options + ['ARCHS=x86_64', 'ONLY_ACTIVE_ARCH=NO'])

  # paths
  root_name = target.pod_name
  module_name = target.product_module_name
  device_framwork_path = "#{build_dir}/#{CONFIGURATION_DEVICE}-#{device}/#{root_name}/#{module_name}.framework"
  simulator_framwork_path = "#{build_dir}/#{CONFIGURATION_SIMULATOR}-#{simulator}/#{root_name}/#{module_name}.framework"
...
module Pod
  class Prebuild
    ...
    def self.build(sandbox_root_path, target, output_path, bitcode_enabled = false)
      ...
      when :osx then xcodebuild(sandbox, target.label, configuration)
